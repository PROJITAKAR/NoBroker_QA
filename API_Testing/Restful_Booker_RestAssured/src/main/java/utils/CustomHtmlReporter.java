...
public class CustomHtmlReporter implements IReporter {
	...
	private String buildHtml(List<ITestResult> passed, List<ITestResult> failed, List<ITestResult> skipped, int total,
			int passRate, String timestamp) {
		...
		sb.append("<div class='summary'>").append(card("Total", String.valueOf(total), "card-total"))
				...
				.append(card("Skipped", String.valueOf(s), "card-skip")).append(card("Pass Rate", passRate + "%",
						passRate == 100 ? "card-pass" : passRate >= 70 ? "card-warn" : "card-fail"))
				.append("</div>");

		// Progress bar
		sb.append("<div class='progress-wrap'>").append("<div class='progress-bar'>")
				.append("<div class='bar-pass'  style='width:").append(pct(p, total)).append("%' title='Passed: ")
				.append(p).append("'></div>").append("<div class='bar-fail'  style='width:").append(pct(f, total))
				.append("%' title='Failed: ").append(f).append("'></div>")
				.append("<div class='bar-skip'  style='width:").append(pct(s, total)).append("%' title='Skipped: ")
				.append(s).append("'></div>").append("</div></div>");

		// Table
		sb.append("<div class='table-wrap'>").append("<h2>Test Case Results</h2>").append("<table>")
				.append("<thead><tr>").append("<th>#</th><th>Feature</th><th>Test Method</th>")
				.append("<th>Status</th><th>Duration (ms)</th><th>Failure Reason</th>").append("</tr></thead><tbody>");
		...
		for (ITestResult r : all) {
			...
			sb.append("<tr class='").append(css).append("'>").append("<td>").append(row++).append("</td>")
					.append("<td>").append(feature(r)).append("</td>").append("<td class='method'>").append(r.getName())
					.append("</td>").append("<td><span class='badge badge-").append(css).append("'>").append(status)
					.append("</span></td>").append("<td>").append(duration).append("</td>")
					.append("<td class='reason'>").append(reason).append("</td>").append("</tr>");
		}
	...
	private String card(String label, String value, String cls) {
		return "<div class='card " + cls + "'><div class='card-val'>" + value + "</div><div class='card-label'>" + label
				+ "</div></div>";
	}
	...
	private String feature(ITestResult r) {
		Feature f = r.getMethod().getConstructorOrMethod().getMethod().getDeclaringClass().getAnnotation(Feature.class);
	...
	private String failReason(ITestResult r) {
		...
		if (msg == null)
			msg = r.getThrowable().getClass().getSimpleName();
	...
	private String css() {
		return "*{box-sizing:border-box;margin:0;padding:0}"
				...
				+ "text-align:center;box-shadow:0 2px 8px rgba(0,0,0,.1)}" + ".card-val{font-size:2rem;font-weight:700}"
				...
				+ "tr:last-child td{border-bottom:none}" + "tr.pass:hover{background:#f0fff4}"
				+ "tr.fail{background:#fff8f8}" + "tr.fail:hover{background:#fff0f0}" + "tr.skip{background:#fffef0}"
				...
				+ ".badge-pass{background:#c6f6d5;color:#276749}" + ".badge-fail{background:#fed7d7;color:#c53030}"
